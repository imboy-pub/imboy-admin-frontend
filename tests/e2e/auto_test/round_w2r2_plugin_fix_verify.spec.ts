...
test('W2R2 复验：PluginManagementPage 5 项修复', async ({ page }) => {
  ...
  await test.step('① 安装弹窗：路径必填标注 + 空路径前端拦截（不发请求）', async () => {
    ...
    await expect(page.getByText('请填写插件路径').first()).toBeVisible({ timeout: 1_000 })
  ...
  await test.step('① 安装正路径：name+path → install 2xx + toast + 列表刷新', async () => {
    ...
    await expect(page.getByText('插件安装成功').first()).toBeVisible({ timeout: 1_000 })
  ...
  await test.step('② installed 态启用：enable 2xx + toast + 徽标已启用', async () => {
    ...
    await expect(page.getByText('插件已启用').first()).toBeVisible({ timeout: 1_000 })
  ...
  await test.step('③ enabled 态禁用：确认弹窗 → disable 2xx + toast + 徽标已禁用', async () => {
    ...
    await expect(page.getByText('插件已禁用').first()).toBeVisible({ timeout: 1_000 })
  ...
  await test.step('清理 location：卸载（确认弹窗 → 2xx + toast，回 installed）', async () => {
    ...
    await expect(page.getByText('插件已卸载').first()).toBeVisible({ timeout: 1_000 })
  ...
  await test.step('⑤ 重置：确认弹窗 → reset 2xx + toast', async () => {
    ...
    await expect(page.getByText('插件已重置').first()).toBeVisible({ timeout: 1_000 })
  ...
  await test.step('④ 强制卸载：再注入失败 → 强制卸载 2xx + toast + 回 installed', async () => {
    ...
    await expect(page.getByText('插件已强制卸载').first()).toBeVisible({ timeout: 1_000 })
