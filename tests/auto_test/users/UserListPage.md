...
| 阻塞 | 需本地存在被封禁测试用户（当前 status=0 共 0 条；先封禁一个测试用户造数即可，解封可逆） | `src/pages/users/UserListPage.tsx` | 「批量解封」操作提交成功并刷新列表数据 | 未测 | 批次3 | 0 | 0 | 0 | 检查过条件仍缺：/users?status=0 返回 0 条；解封本身可逆可自动化，造数后转待复验；删除/解散/踢出仍人工 |
