...
| 无待办 | - | `src/pages/roles/RolePermissionPage.tsx` | 「角色创建」操作提交成功并刷新列表数据 | 已通过 | 批次3 | 0 | 0 | 0 | 自动填表提交 2xx |
| 无待办 | - | `src/pages/roles/RolePermissionPage.tsx` | 「角色权限已保存」操作提交成功并刷新列表数据 | 已通过 | 批次3 | 0 | 0 | 0 | 自动填表提交 2xx |
| 阻塞 | 需人工验证 | `src/pages/roles/RolePermissionPage.tsx` | 「角色已停用」操作提交成功并刷新列表数据 | 未测 | 批次3 | 0 | 0 | 0 | 写操作交互深度超出自动化边界（icon 行内按钮/编辑弹窗） |
| 阻塞 | 需人工验证 | `src/pages/roles/RolePermissionPage.tsx` | 「角色已删除」操作提交成功并刷新列表数据 | 未测 | 批次3 | 0 | 0 | 0 | 写操作交互深度超出自动化边界（icon 行内按钮/编辑弹窗） |
