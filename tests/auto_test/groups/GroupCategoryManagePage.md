...
| 阻塞 | 永久人工道（§1.4 不可逆/敏感写操作） | `src/pages/groups/GroupCategoryManagePage.tsx` | 危险/写操作二次确认弹窗（确认执行与取消） | 未测 | 批次3 | 0 | 0 | 0 | 确认执行路径属不可逆/敏感操作 |
| 阻塞 | 永久人工道（§1.4 不可逆写操作） | `src/pages/groups/GroupCategoryManagePage.tsx` | 「分类已删除」操作提交成功并刷新列表数据 | 未测 | 批次3 | 0 | 0 | 0 | 行内操作含不可逆动作（删除/解散/踢出），按规程人工执行 |
